...
ways_num = size(way.id, 2); %cantidad de calles en el mapa
...
%-----------MODIFICACION HAVERSINE----------------
for k=1:size(connectivity_matrix,1)
    ind_col=find(connectivity_matrix(:,k)==1);
    if(length(ind_col)>0)
        LatLon_ini = parsed_osm.node.xy(:, k);
        for l=1:length(ind_col)
            LatLon_fin = parsed_osm.node.xy(:, ind_col(l));
            if(haversine(LatLon_ini(2), LatLon_ini(1), LatLon_fin(2), LatLon_fin(1))>0.150)
                connectivity_matrix(ind_col(l),k)=0;
            end
        end
    end
end
...
% figure; spy(connectivity_matrix)
