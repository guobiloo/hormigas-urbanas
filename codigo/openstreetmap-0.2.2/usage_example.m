...
clear all
close all
addpath(genpath('./dependencies')); %agregar las dependencias necesarias para correr esta libreria

%% name file
openstreetmap_filename = 'mapa_comun.osm';
...
plot_route(ax, route, parsed_osm,'r','-'); %mostrar la ruta optima encontrada
only_nodes = 1:1:1000;
% plot_nodes(ax, parsed_osm, only_nodes) %mostrar absolutamente todos los nodos

% show intersection nodes (unseful, but may result into a cluttered plot)
plot_nodes(ax, parsed_osm, intersection_node_indices) %solo mostrar esquinas y su id asociada
