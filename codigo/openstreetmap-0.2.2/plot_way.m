...
function [] = show_ways(hax, bounds, node, way, map_img_filename)
    ...
    for i=1:size(way.id, 2)
        ...
        if flag == 1
            ...
            %-----------DIBUJAR FLECHAS PARA INDICAR SENTIDOS (comentar la linea anterior)-------------
%                     for k=1:length(nd_coor(1,:))-1
%                         arrow(nd_coor(:,k),nd_coor(:,k+1),4,'color','b');
%                     end
            %-----------DIBUJAR FLECHAS PARA INDICAR SENTIDOS-------------
        else
            %         plot(hax, nd_coor(1,:), nd_coor(2,:), 'g--') %dibujar caminos que no son calles(vias de tren por ej)
        end
        %waitforbuttonpress
    end
